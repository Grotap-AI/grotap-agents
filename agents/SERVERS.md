...
|---|---|---|---|---|
| agent-02-claude | 5.161.74.39 | cpx21, Ashburn | Intake, Triage, Security Reviewer | 3 |
| agent-03-claude | 5.161.81.193 | cpx21, Ashburn | Planner, Fix/Logic/Policy/Perf Reviewer | 3 |
| agent-04-claude | 178.156.222.220 | cpx21, Ashburn (ID 122714167). OOM-froze 7/5 (hard reset); keep swap enabled | Execute, Change Reviewer, Rule Enforcer, Build Validator | 3 |
| agent-05-claude | 5.161.73.195 | cpx21, Ashburn | Pipeline Detail, Audit Filters, Mobile Approvals, Marketing (consolidated from agent-11 4/29: grotap.com site, Meta/Instagram/Facebook, YouTube, TikTok APIs) | 3 |
| agent-06-claude | 5.161.53.103 | cpx31, Ashburn | **Deploy Ops + pipeline monitoring** (see below) | 2 |

Cloud names locked 2026-09-21 (Hetzner already renamed). SSH aliases in this repo match those names.
`agent-01` is **not** in this live pool. Do not create `agent-11-codex`. Do not provision
`prompt-01-astra` or `agent-team-01-astra` from this roster. `claude-code-01` stays that name
(not `prompt-01-claude`).
...
(`~/.ssh/grotap_from06_agent-0{2,3,4,5}` — basenames still the pre-rename `agent-0N` form) plus matching
`Host` blocks in both of its `~/.ssh/config` files; agent-02-claude…agent-05-claude carry the matching
public halves. `agents/scripts/ssh-key-for.sh` resolves the new cloud name first and falls back to that
pre-rename basename, so existing keys keep working. Phase 2b (same day): every fleet
...
|---|---|---|---|---|
| agent-10-codex | 87.99.148.22 | cpx21, Ashburn (id 148646754) | Team 2 open-model executor (Codex via OpenRouter). Formerly `agent-20`. | 3 |
...
| Server | IP | Hardware / DC | Purpose | Execute slots |
|---|---|---|---|---|
| monitor-01-deepseek | 178.156.219.232 | cpx21, Ashburn (id 150671577) | Team 4 GPT-5.6 executor. Formerly `agent-40`. Still shared by team4, team5, and `MONITOR_POOL`. | 3 |
...
> siblings agent-10-codex and monitor-01-deepseek carry the work and stay (agent-30 followed them on 2026-09-20).
...
| Host | IP | Purpose |
|---|---|---|
| openreplay-01 | 5.161.189.143 | **OpenReplay session-replay server** — `supportagents.grotap.com`. Formerly `grotap-cobrowse-01` (also called supportagents in older notes). Rescaled to cpx51 + wiped 2026-07-07 (OpenReplay Docker stack, projectId 2); replaces Cobrowse.io (`setup-cobrowse-runner.sh` obsolete; cobrowse-runner on this host is disabled). Still in Hetzner project `Cobrowse.Grotap.com` (`HETZNER_COBROWSE_API_TOKEN`). ⚠ IP recycled twice (deleted agent-01 → cobrowse host → this) — stale host-key warnings: `ssh-keygen -R 5.161.189.143`. |
...
| openreplay-ai-support | 178.156.199.83 | **OpenReplay AI assist session runner — KEEP, do not retire.** Formerly `grotap-runner-01`. Headless-Chromium OpenReplay Assist driver. cpx21 Ashburn (id 149246895), Hetzner project `Cobrowse.Grotap.com` (`HETZNER_COBROWSE_API_TOKEN`), firewall `runner-fw` (inbound ssh+icmp only — runner is outbound-only). Provisioned 2026-07-09. `RUNNER_ID=openreplay-ai-support`. Deploy: git pull + `npm ci && npm run build` in `~/grotap-platform/cobrowse-agent-runner`, then restart the support-runner unit (restart does NOT rebuild). |
...
- **agent-01** (5.161.189.143, deleted 6/29 — that IP is now `openreplay-01`, not a Claude worker) · **agent-07** (89.167.66.105, gone with cancelled account) · **agent-08** (77.42.42.213, deleted; old dispatch box, role moved to what is now agent-06-claude on 4/29).
- **Hillsboro agent-06 address `5.78.178.81`** — released. Live agent-06-claude is Ashburn `5.161.53.103`. Never SSH the old address, never put it in a firewall allowlist.
...
1. `ssh -i ~/.ssh/grotap_agents root@<IP>`; add an alias to `~/.ssh/config` using the Hetzner cloud name (`agent-02-claude`, not `agent-02`). A brand-new box has no per-host key yet, so this first-contact step always uses the shared fleet key regardless of the SSH key retirement below — that's expected, not a regression.
