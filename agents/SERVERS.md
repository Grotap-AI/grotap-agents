...
- agent cron: `pipeline_failure_monitor.py` 10m · `deploy_freshness_watchdog.py` 5m · **review gate every 15 min** (`review-gate-cron.sh`; empty queue exits <5s). `review-gate.timer` is active on this host only. `review-gate-cron.sh` exits unless `hostname -s` is `agent-06-claude` (this Linux hostname, not the SSH alias `agent-06`). agent-01-claude must not run `review-gate.timer` or `review-gate.service`. This repo has no unit or installer for that timer; the host guard is what stops a second copy.
