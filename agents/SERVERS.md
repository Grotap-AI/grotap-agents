...
**`prompt-01-claude`** at `178.156.209.112` (former `claudecode-01`, then `claude-code-01`; DNS `claudecode.grotap.com` may still point here).
...
Verified against live Hetzner Cloud names and Linux hostnames (Shadow Harness, 2026-09-24).
The Cloud name is the Hetzner column. `SSH alias` is not a Hetzner name. Roles stay on the IP.
`5.161.189.143` is `openreplay-01`, not `agent-01-claude`. Only `agent-05-claude` is powered off.
...
| agent-05-claude | 5.78.178.81 | (powered off; Hillsboro) | `agent-05` | Hillsboro. **OFF / unreachable.** This is the powered-off Hillsboro box | Deploy Ops + pipeline monitoring were installed here (see below). Do not dispatch while it is off | 2 |
| agent-06-claude | 5.161.53.103 | **still `grotap-agent-06-ash`** — set to `agent-06-claude` when SSH works | `agent-06` | Ashburn. **Running**. Not the Hillsboro box | Team Claude worker (was `agent-06-ash`) | 2 |
...
| Cloud name | IP | Role |
|---|---|---|
| prompt-01-astra | 5.161.243.18 | Prompt Option — GPT-6 Astra (`gpt-6-astra`). cpx31 Ashburn, Hetzner id 167204705. GO 2026-09-23. Outside the Team Claude pool. |
| agent-team-01-astra | 5.161.80.75 | Agent Team — GPT-6 Astra (`gpt-6-astra`). cpx31 Ashburn, Hetzner id 167204706. GO 2026-09-23. Outside the Team Claude pool. |
