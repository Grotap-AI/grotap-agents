...
Live Ashburn, outside the Team Claude (team1) pool: **`prompt-01-astra`** `5.161.243.18` (Hetzner id 167204705), **`agent-team-01-astra`** `5.161.80.75` (id 167204706)
(`gpt-6-astra`, cpx31) — Team Astra (team5) seats, not dispatch executors. Do not create `agent-11-codex`. Do not retire `openreplay-ai-support`.
...
The retired FSN1 box on the unused `team3` key also proved the second cost of EU placement — grok-4.5 is xAI-region-blocked
from EU egress, which is why that slot had to run a different, weaker model ladder.
...
## Team Codex (team2) — open-model executors (Hetzner project **OpenAgents.grotapai**, token `HETZNER_FARM_API_TOKEN`)
Provisioned 2026-07-07 (cases AA8CFD/F404F8/959C5E). Aider + OpenRouter runtime —
NO claude CLI on these boxes by design. ⚠ NOT in the Team Claude dispatch pool: do not dispatch until the
...
|---|---|---|---|---|
| agent-10-codex | 87.99.148.22 | cpx21, Ashburn (id 148646754). Cloud name `agent-10-codex`. Linux hostname may still be `agent-20` | Team Codex — GPT Codex via OpenRouter | 3 |
...
4 GiB swap, `~/grotap-agents` + `~/grotap-platform` clones, `~/worktrees`. Scale Team Codex by adding
boxes here + the team2 pool in `config.sh` — box CPU is only builds/git, inference is remote (OpenRouter/Lane C).

## `team3` — reserved, unused

`team3` is reserved and unused. It is an internal key with no servers, and `TEAM_POOL` for `team3` is empty.

> **Retired 2026-09-20:** `agent-30` (150155427) was deleted from Hetzner
...
> put it in a firewall allowlist. Re-provision only with an owner decision, via `agents/provision-team3.py`.

## Team Monitor (DeepSeek, team4) — DeepSeek via OpenRouter (Hetzner project **OpenAgents.grotapai**, token `HETZNER_FARM_API_TOKEN`)
Provisioned 2026-07-13 for the GPT-5.6 hedge (cases B8EF68/FC0208/BA9BC6/8B9BFC —
...
Team Astra note: the Codex CLI runner experiment (case 8B9BFC) SHARES this box by design
(experiment phase; 3 slots/box, inference remote) — revisit if both teams go live.

| Server | IP | Hardware / DC | Purpose | Execute slots |
|---|---|---|---|---|
| monitor-01-deepseek | 178.156.219.232 | cpx21, Ashburn (id 150671577). Cloud name `monitor-01-deepseek`. Linux hostname may still be `agent-40` | Team Monitor (DeepSeek) — DeepSeek via OpenRouter (provisioned as the GPT-5.6 executor) | 3 |

Baseline identical to Team Codex (agent user, node 22, doppler + `git-credential-doppler`, aider
...
> `5226d7fc9`) and EMPTY for team3 since 2026-09-20. `team3` stays reserved and unused.
> Rebuild specs if that key is ever given metal: cpx21 (cpx22 for FSN1), ubuntu-24.04,
> labels `role=<open-model|grok|gpt>-executor`, `team=team<N>`;
...
## Prompt / Team Astra — not in the Team Claude pool

Not dispatch executors. Not in `config.sh` pools. `5.161.243.18` was released with deleted `agent-21` and is now this Team Astra host — use the Cloud name, not `agent-21`.

| Cloud name | IP | Role |
|---|---|---|
| prompt-01-astra | 5.161.243.18 | Team Astra prompt seat — GPT-6 Astra (`gpt-6-astra`). **Running.** cpx31 Ashburn, Hetzner id 167204705. Outside the Team Claude pool. |
| agent-team-01-astra | 5.161.80.75 | Team Astra agent seat — GPT-6 Astra (`gpt-6-astra`). **Running.** cpx31 Ashburn, Hetzner id 167204706. Outside the Team Claude pool. |
