...
- Review gates 2026-07-07→09, condensed (full text: agents/LESSONS-ARCHIVE.md) — D3117A/4680FB: a frontend-only branch consuming a NEW endpoint silently 404s if the backend isn't on master — grep master backend for the route first, land the backend sibling FIRST; a case marked `done` may never have merged (verify `git branch -r --contains`). EA3E8D: a BLOCK that lives only in prose is NOT a block — make it machine-visible FIRST (flip dispatch row to `rejected` / park the case) BEFORE writing any report, or the auto-approve cron merges it. DC3D27+04473B: parallel cases both took `v037_*` — check `git ls-tree origin/master` + peer branches before picking vNNN; reviewer confirms distinct versions on any ≥2-migration batch. · D7A410+8DC100 (recurrence, SAME app/day): build-breaking frontend branches shipped TS6196/TS2322/TS2304 — a frontend deliverable is DONE only when the UI renders AND calls its endpoints, and `cd frontend && npx tsc --noEmit` MUST pass before claiming done, every time. · 57E1DC/E8600B (recurrence of 05572B/8584FE): sibling subtasks again rewrote the same function + duplicated a migration — siblings own DISJOINT files/functions, grep CURRENT master for the symbol and wire to it; AND a dedup is a spec-compliance decision — diff both versions against the spec before discarding (spec said `secrets`; the surviving inline version used `random` — never acceptable for codes/tokens/session ids; fixed at gate).
