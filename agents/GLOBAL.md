...
- ORP dual-run (until ORP3+ORP5 land): AI-support session-start must send BOTH `cobrowse_session_id` AND `openreplay_session_id` when each is available — NEVER omit the cobrowse id because OpenReplay is live (the runner hard-throws "claim missing cobrowse_session_id" → 100% AI-help failure). 3C35FC's origin rebuild (4967627c) did exactly this and was REJECTED at the gate; the correct build is `case-CASE-20260707-3C35FC-attempt2` (dual-send via `lib/openreplaySession.ts` seam). Also: the filename `frontend/src/lib/openreplay.ts` is RESERVED for ORP1's tracker wrapper — never create it for plumbing/seams.
...
- Review gate 2026-07-08 (EA3E8D): the ORP5 Cobrowse rip-out keeps getting re-cut prematurely — this time as a subtask (parent 459DB3) that actually WROTE harmful code (removed HelpMenu dual-send + deleted `frontend/src/lib/cobrowse.ts`) instead of a marker doc. Same defect the dual-run lesson (above) warns of: with ORP3 unshipped, `cobrowse-agent-runner/src/index.ts:52-53` still hard-throws `claim missing cobrowse_session_id`, so dropping the dual-send = 100% AI-help failure. BLOCKED, not merged; block case A976EF filed. Before touching ANY Cobrowse-removal case, verify the ORP5 gate on CURRENT master: ORP3 (9C3809) merged AND runner index.ts no longer throws at 52-53 AND owner confirmed Assist live in prod. All false → the case is premature; ship a marker doc / hold, never a code deletion. Precedent chain: 085AAA·E47AC8·9E6FFE·B28CD4·2197B9·D6EC58·AC3753.
