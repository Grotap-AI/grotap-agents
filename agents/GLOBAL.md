...
- Review gate 2026-07-09 (E8600B, follow-on to the 57E1DC dedup) — **when you drop a duplicate hunk, re-check the SURVIVOR against the spec.** The parent spec (0A3533) mandated share-code generation "with the secrets module"; sibling 57E1DC built exactly that, but was rejected as a structural duplicate of E8600B's earlier inline version — which used `random.choices` (non-cryptographic). Dropping the duplicate silently kept the spec-violating implementation on master. Rules: (1) agents: spec says `secrets`, use `secrets` — `random` is never acceptable for codes/tokens/session ids, even short-lived ones; (2) reviewers: a dedup decision is also a spec-compliance decision — before discarding the "duplicate", diff BOTH versions against the spec line and keep/port the compliant behavior (here: `secrets.choice` + the status='waiting' collision filter). Fixed at the review gate (one-line `random`→`secrets` swap in support_live.py).
