...
- **Before filtering/querying a 3rd-party system by a key, prove WE emit that key.** A LangSmith proxy filtered runs on `metadata.case_id` — the orchestrator only sets `thread_id`; endpoint returns `[]` forever. `git grep` the producing side for the key (and verify the API's actual request/response shape from current docs) before building the consumer.
