...
- D7A410 + 8DC100 (recurrence, SAME app/day): two build-breaking frontend branches shipped unused interfaces (TS6196), undeclared JSX props (TS2322), and a rendered-but-never-defined component (TS2304) — a frontend deliverable is DONE only when the UI renders AND calls its endpoints, and `cd frontend && npx tsc --noEmit` MUST pass before claiming done, every time. 57E1DC/E8600B (recurrence of 05572B/8584FE): sibling subtasks again rewrote the same function + duplicated a migration — siblings own DISJOINT files/functions, grep CURRENT master for the symbol and wire to it; AND a dedup is a spec-compliance decision — diff both versions against the spec before discarding (spec said `secrets`; the surviving inline version used `random` — never acceptable for codes/tokens/session ids; fixed at gate).
