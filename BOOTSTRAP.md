...
## Never Do
- SSH by raw IP — use the canonical alias in `agents/SERVERS.md` (`ssh agent-02-claude`, `ssh agent-10-codex`, `ssh monitor-01-deepseek`, `ssh prompt-01-claude`). `agent-NN` is not the name. Key: `~/.ssh/grotap_agents`
