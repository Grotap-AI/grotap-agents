...
ssh agent-06-claude        # 5.161.53.103    (User: root)  SSH alias agent-06; hostname still grotap-agent-06-ash
...
4. **SSH from the ops box** (`agent-05-claude`, Hillsboro, currently off; agent-08 is retired): Needs `~/.ssh/grotap_agents` key + `~/.ssh/config`. Prefer Cloud-name `Host` aliases.
...
| agent-05-claude | root only | OK | Hillsboro ops. OFF. SSH alias `agent-05` |
| agent-06-claude | root only | OK | Ashburn. Hostname still `grotap-agent-06-ash`. SSH alias `agent-06` |
